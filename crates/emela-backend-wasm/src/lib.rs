...
/// Which WASM target a core module is emitted for. `Wasi` is the preview1 core
/// module `emela run` (wasmi) executes, with the custom `emela_socket`/
/// `emela_http` host imports; `Wasip2` is the core module the component backend
/// (`emela-backend-wasm-wasip2`) wraps into a WASI 0.2 component — it uses a
/// `run` entry (no preview1 `_start`/`proc_exit`). The platform-function glue is
/// added target by target (spec 0052); `Wasip2` currently supplies none.
...
impl WasmTarget {
    fn label(self) -> &'static str {
        match self {
            ...
            WasmTarget::Wasip2 => "wasm-wasip2",
        }
    }
}

/// Emits the core WASM module (WAT text) for `ir`, targeting `target`. The
/// `Wasip2` core is meant to be wrapped into a component by the caller.
pub fn emit_module(
    ir: &IrProgram,
    platform_registry: &[emela_codegen::PlatformFn],
    target: WasmTarget,
    ...
    for name in &used_platform {
        // `Wasip2` does not yet supply any platform function (spec 0052; the
        // `wasi:cli`/`wasi:sockets` glue lands stage by stage). `Wasi` supplies
        // the historical set via `platform_glue`/`host_platform_provided`.
        let provided = match target {
            WasmTarget::Wasi => {
                platform_glue(name).is_some() || host_platform_provided(name, platform_registry)
            }
            // `Wasip2` supplies the WASI 0.2 leaves stage by stage (spec 0052).
            WasmTarget::Wasip2 => {
        ...
        if !provided {
            return Err(BackendError::new(format!(
                "backend `{}` does not provide platform function `{name}`",
                target.label()
            )));
        }
    }
    ...
    for name in used_intrinsics(ir) {
        if !wasm_provides_intrinsic(&name) {
            return Err(BackendError::new(format!(
                "backend `{}` does not provide intrinsic `{name}`",
                target.label()
            )));
        }
    }
    ...
    // The `Wasi` core is a preview1 command (`_start` traps via `proc_exit`);
    // the `Wasip2` core has no preview1 surface — it exports `run` and the
    // component wrapper supplies WASI 0.2 (spec 0052).
    ...
    match target {
        ...
        WasmTarget::Wasip2 => module.push_str(&wasip2_imports(&used_platform)),
    }
    let _ = writeln!(
        module,
    ...
    for name in &used_platform {
        match target {
            ...
            WasmTarget::Wasip2 => {
                if let Some(glue) = wasip2_platform_glue(name) {
                    module.push_str(glue);
                }
            }
        }
    }
    // The Socket glue shares helpers across its five operations, so it is
    // emitted once rather than per used name (spec 0050/0052).
    ...
    match target {
        ...
        WasmTarget::Wasip2 => {
            // The canonical-ABI allocator wit-component uses to lift host-owned
            // values (e.g. the `list<u8>` a `socket.read` returns) into guest
            // memory. Backed by the RC allocator's leak-drop path.
            module.push_str(
                "  (func $cabi_realloc (param i32 i32 i32 i32) (result i32)\n    local.get 3\n    call $alloc_host)\n  (export \"cabi_realloc\" (func $cabi_realloc))\n",
            );
            // The core export the component's `wasi:cli/run` interface lifts
            // (spec 0052). The `<iface>#<func>` name is the canonical-ABI export
            // convention wit-component matches against the world.
            module.push_str(&emit_run(main));
            module.push_str("  (export \"wasi:cli/run@0.2.0#run\" (func $run))\n");
        }
    }
    if target == WasmTarget::Wasi
...
/// The WASI 0.2 core imports the `Wasip2` core module needs for the platform
/// functions it uses (spec 0052). Names follow the canonical-ABI convention so
/// wit-component wires them to the world's imports. The stream write/drop are
/// shared by stdout and stderr, so they are emitted once.
fn wasip2_imports(used: &[String]) -> String {
    ...
    if uses_stdout || uses_stderr || uses_socket {
        out.push_str(
            "  (import \"wasi:io/streams@0.2.0\" \"[method]output-stream.blocking-write-and-flush\" (func $stream_write (param i32 i32 i32 i32)))\n",
        );
        out.push_str(
            "  (import \"wasi:io/streams@0.2.0\" \"[resource-drop]output-stream\" (func $os_drop (param i32)))\n",
        );
    }
    if uses_stdout {
        out.push_str(
    ...
    if uses_stderr {
        out.push_str(
            "  (import \"wasi:cli/stderr@0.2.0\" \"get-stderr\" (func $get_stderr (result i32)))\n",
        );
    }
    if uses_socket {
...
/// The `wasi:sockets`/`wasi:io` core imports (canonical-ABI signatures) for the
/// `Socket` capability (spec 0050/0052). `start-bind` takes the flattened
/// `ip-socket-address` (disc + 11 payload words); every fallible call takes a
/// trailing return-area pointer and writes its `result` there.
const WASIP2_SOCKET_IMPORTS: &str = "\
  (import \"wasi:sockets/instance-network@0.2.0\" \"instance-network\" (func $wsi_net (result i32)))
  (import \"wasi:sockets/network@0.2.0\" \"[resource-drop]network\" (func $wsi_net_drop (param i32)))
  (import \"wasi:sockets/tcp-create-socket@0.2.0\" \"create-tcp-socket\" (func $wsi_create (param i32 i32)))
  (import \"wasi:sockets/tcp@0.2.0\" \"[method]tcp-socket.start-bind\" (func $wsi_start_bind (param i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32 i32)))
  (import \"wasi:sockets/tcp@0.2.0\" \"[method]tcp-socket.finish-bind\" (func $wsi_finish_bind (param i32 i32)))
  (import \"wasi:sockets/tcp@0.2.0\" \"[method]tcp-socket.start-listen\" (func $wsi_start_listen (param i32 i32)))
  (import \"wasi:sockets/tcp@0.2.0\" \"[method]tcp-socket.finish-listen\" (func $wsi_finish_listen (param i32 i32)))
  (import \"wasi:sockets/tcp@0.2.0\" \"[method]tcp-socket.accept\" (func $wsi_accept (param i32 i32)))
  (import \"wasi:sockets/tcp@0.2.0\" \"[method]tcp-socket.subscribe\" (func $wsi_subscribe (param i32) (result i32)))
  (import \"wasi:sockets/tcp@0.2.0\" \"[resource-drop]tcp-socket\" (func $wsi_tcp_drop (param i32)))
  (import \"wasi:io/poll@0.2.0\" \"[method]pollable.block\" (func $wsi_block (param i32)))
...
fn wasip2_platform_glue(canonical: &str) -> Option<&'static str> {
    match canonical {
...
// then drop the stream handle. The `result<_, stream-error>` lands in the
// 16-byte scratch at offset 0 and is ignored (best-effort, like preview1
// `fd_write`). `$s+4` is the byte pointer, `[$s]` the byte length.
const WASIP2_WRITE_STDOUT_GLUE: &str = "  (func $plat_io_write_stdout (param $s i32) (result i32)\n    (local $stream i32)\n    call $get_stdout\n    local.set $stream\n    local.get $stream\n    local.get $s i32.const 4 i32.add\n    local.get $s i32.load\n    i32.const 0\n    call $stream_write\n    local.get $stream\n    call $os_drop\n    i32.const 0)\n";

const WASIP2_WRITE_STDERR_GLUE: &str = "  (func $plat_io_write_stderr (param $s i32) (result i32)\n    (local $stream i32)\n    call $get_stderr\n    local.set $stream\n    local.get $stream\n    local.get $s i32.const 4 i32.add\n    local.get $s i32.load\n    i32.const 0\n    call $stream_write\n    local.get $stream\n    call $os_drop\n    i32.const 0)\n";

/// The `Socket` capability glue (spec 0050/0052) over `wasi:sockets`. Emitted
/// once when any `socket.*` is used. A `Listener`/`Connection` `id` is a pointer
/// to a 16-byte handle block `[kind][h0][h1][h2]` (kind 1 = listener with a
/// `tcp-socket`; kind 2 = connection with `tcp-socket`, `input-stream`,
/// `output-stream`). The 16-byte scratch at offset 0 is the return area for the
/// fallible calls. Every error maps to `SocketError::ConnectionClosed` (tag 2);
/// the callers (e.g. the HTTP handler) collapse SocketError anyway (spec 0046).
fn wasip2_socket_glue() -> &'static str {
    WASIP2_SOCKET_GLUE
}

const WASIP2_SOCKET_GLUE: &str = r#"  (func $sock_err (result i32)
    (local $e i32) (local $c i32)
    i32.const 8 call $alloc_host local.set $e
    ...
    local.get $c)
  (func $sock_ok_handle (param $blk i32) (result i32)
    (local $r i32) (local $c i32)
    i32.const 8 call $alloc_host local.set $r
    local.get $r local.get $blk i32.store
    i32.const 16 call $alloc_host local.set $c
    local.get $c i32.const 1 i32.store
    local.get $c i32.const 8 i32.add local.get $r i32.store
    local.get $c)
  (func $sock_ok_val (param $v i32) (result i32)
    (local $c i32)
    i32.const 16 call $alloc_host local.set $c
    local.get $c i32.const 1 i32.store
    local.get $c i32.const 8 i32.add local.get $v i32.store
    local.get $c)
  (func $plat_socket_raw_listen (param $port i32) (result i32)
    (local $net i32) (local $sock i32) (local $blk i32)
    call $wsi_net local.set $net
    i32.const 0 i32.const 0 call $wsi_create
    i32.const 0 i32.load
    if local.get $net call $wsi_net_drop call $sock_err return end
    i32.const 4 i32.load local.set $sock
    local.get $sock local.get $net
    i32.const 0 local.get $port i32.const 0 i32.const 0 i32.const 0 i32.const 0
    i32.const 0 i32.const 0 i32.const 0 i32.const 0 i32.const 0 i32.const 0
    i32.const 0
    call $wsi_start_bind
    i32.const 0 i32.load
    if local.get $net call $wsi_net_drop local.get $sock call $wsi_tcp_drop call $sock_err return end
    local.get $sock i32.const 0 call $wsi_finish_bind
    i32.const 0 i32.load
    if local.get $net call $wsi_net_drop local.get $sock call $wsi_tcp_drop call $sock_err return end
    local.get $sock i32.const 0 call $wsi_start_listen
    i32.const 0 i32.load
    if local.get $net call $wsi_net_drop local.get $sock call $wsi_tcp_drop call $sock_err return end
    local.get $sock i32.const 0 call $wsi_finish_listen
    i32.const 0 i32.load
    if local.get $net call $wsi_net_drop local.get $sock call $wsi_tcp_drop call $sock_err return end
    local.get $net call $wsi_net_drop
    i32.const 16 call $alloc_host local.set $blk
    local.get $blk i32.const 1 i32.store
    local.get $blk i32.const 4 i32.add local.get $sock i32.store
    local.get $blk call $sock_ok_handle)
  (func $plat_socket_raw_accept (param $lrec i32) (result i32)
    (local $blk i32) (local $sock i32) (local $poll i32) (local $cblk i32)
    local.get $lrec i32.load local.set $blk
    local.get $blk i32.const 4 i32.add i32.load local.set $sock
    local.get $sock call $wsi_subscribe local.set $poll
    local.get $poll call $wsi_block
    local.get $poll call $wsi_poll_drop
    local.get $sock i32.const 0 call $wsi_accept
    i32.const 0 i32.load
    if call $sock_err return end
    i32.const 16 call $alloc_host local.set $cblk
    local.get $cblk i32.const 2 i32.store
    local.get $cblk i32.const 4 i32.add i32.const 4 i32.load i32.store
    local.get $cblk i32.const 8 i32.add i32.const 8 i32.load i32.store
    local.get $cblk i32.const 12 i32.add i32.const 12 i32.load i32.store
    local.get $cblk call $sock_ok_handle)
  (func $plat_socket_raw_read (param $crec i32) (param $max i32) (result i32)
    (local $blk i32) (local $in i32) (local $lptr i32) (local $llen i32) (local $b i32)
    local.get $crec i32.load local.set $blk
    local.get $blk i32.const 8 i32.add i32.load local.set $in
    ...
    local.get $b local.get $llen i32.store
    local.get $b i32.const 4 i32.add local.get $lptr local.get $llen memory.copy
    local.get $b call $sock_ok_val)
  (func $plat_socket_raw_write (param $crec i32) (param $data i32) (result i32)
    (local $blk i32) (local $out i32)
    local.get $crec i32.load local.set $blk
    local.get $blk i32.const 12 i32.add i32.load local.set $out
    local.get $out local.get $data i32.const 4 i32.add local.get $data i32.load i32.const 0 call $stream_write
    i32.const 0 i32.load
    if call $sock_err return end
    i32.const 0 call $sock_ok_val)
  (func $plat_socket_raw_close (param $blk i32) (result i32)
    local.get $blk i32.load i32.const 1 i32.eq
    if
      local.get $blk i32.const 4 i32.add i32.load call $wsi_tcp_drop
    else
      ;; The `input-stream`/`output-stream` are children of the `tcp-socket`;
      ;; wasi:sockets requires dropping them before the socket ("resource has
      ;; children" otherwise).
      local.get $blk i32.const 8 i32.add i32.load call $wsi_in_drop
...
/// The `Wasip2` entry (spec 0052): the component's `wasi:cli/run` export lifts
/// this. It returns the `result<_, _>` discriminant as an `i32` — `0` = ok, `1`
/// = err. There is no `proc_exit`; a nonzero `Int` result maps to err (a full
/// exit-code path via `wasi:cli/exit` is a later refinement).
fn emit_run(main: &IrFunction) -> String {
    let mut out = String::new();
    out.push_str("  (func $run (result i32)\n");
    out.push_str("    i32.const 0\n");
    out.push_str("    call $f_main\n");
    if main.ret == Type::Int {
        // Nonzero exit code -> err(1), zero -> ok(0).
        out.push_str("    i32.const 0\n    i32.ne)\n");
    } else if is_heap(&main.ret) {
        out.push_str("    call $rc_release\n    i32.const 0)\n");
    } else {
